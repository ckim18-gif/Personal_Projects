...
slash_active = False
slash_timer = 0
slash_duration = 18           # Frames for a single strike arc (about 0.3s)

# Passive - Double Strike (every 4th basic attack swings twice: down then up)
attack_count = 0
...
# Smoothed body pose (lerped every frame so attacks can be canceled/blended
# into other animations without a jarring snap back to rest)
cur_head_rot = 0.0
cur_left_arm_rot = 0.0
cur_right_arm_rot = 0.0
cur_left_leg_rot = 0.0
cur_right_leg_rot = 0.0
cur_sword_yaw = 0.0
...
def lerpAngle(current, target, amt):
    """Simple linear interpolation used to smoothly blend body pose each frame"""
    return current + (target - current) * amt

def triggerSlash():
    """Starts a basic attack. Tracks Master Yi's passive: every 4th attack is a Double Strike."""
    global slash_active, slash_timer, current_anim, meditate_active, attack_count, double_strike_active
    if meditate_active:
        meditate_active = False
    slash_active = True
    slash_timer = 0
    current_anim = "slash"
    attack_count += 1
    double_strike_active = (attack_count % 4 == 0)

def cancelSlash():
    """Interrupts an in-progress basic attack (attack-cancel). The pose lerp
    handles blending smoothly out of whatever swing frame we were on."""
    global slash_active, slash_timer, double_strike_active
    slash_active = False
    slash_timer = 0
    double_strike_active = False
...
def draw():
    ...
    global slash_active, slash_timer, current_anim, double_strike_active
    global cur_head_rot, cur_left_arm_rot, cur_right_arm_rot, cur_left_leg_rot, cur_right_leg_rot, cur_sword_yaw

    # Target pose for this frame - the actual pose used for rendering is
    # smoothed (lerped) towards this target, not snapped directly to it.
    target_head_rot = 0
    target_left_arm_rot = 0
    target_right_arm_rot = 0
    target_left_leg_rot = 0
    target_right_leg_rot = 0
    target_sword_yaw = 0
    ...
    if alpha_active:
        ...
        target_head_rot = breathe * 0.5
        target_right_arm_rot = -1.3 + breathe
        target_left_arm_rot = -1.3 - breathe
    elif slash_active:
        slash_timer += 1
        duration = slash_duration * 2 if double_strike_active else slash_duration

        # End of the attack (single strike, or both hits of a Double Strike)
        if slash_timer >= duration:
            slash_active = False
            slash_timer = 0
            double_strike_active = False
            current_anim = "idle"
        ...
        if double_strike_active:
            # Passive - Double Strike: first hit chops down, second hit
            # immediately follows with a reversed upward swing.
            half = duration / 2.0
            if slash_timer < half:
                pp = slash_timer / half
                strike_factor = sin(pp * PI)
                target_right_arm_rot = -1.6 + strike_factor * 2.4   # downward chop
                target_left_arm_rot = -1.4 + strike_factor * 2.4
                target_sword_yaw = strike_factor * 0.6
            else:
                pp = (slash_timer - half) / half
                strike_factor = sin(pp * PI)
                target_right_arm_rot = 0.8 - strike_factor * 2.6    # upward reverse swing
                target_left_arm_rot = 1.0 - strike_factor * 2.6
                target_sword_yaw = -strike_factor * 0.6
        else:
            p = float(slash_timer) / duration
            strike_factor = sin(p * PI)
            target_right_arm_rot = -1.6 + strike_factor * 2.4
            target_left_arm_rot = -1.4 + strike_factor * 2.4
            target_sword_yaw = strike_factor * 0.6

        target_left_leg_rot = 0.4
        target_right_leg_rot = -0.4
        target_head_rot = 0.15
    elif current_anim == "walk":
        target_left_leg_rot = sin(t_walk) * 0.8
        target_right_leg_rot = -sin(t_walk) * 0.8
        target_right_arm_rot = -1.0 + sin(t_walk * 2) * 0.1
        target_left_arm_rot = -0.8 + sin(t_walk * 2) * 0.1
        target_head_rot = sin(t_walk * 2) * 0.04
    else:
        breathe = sin(frameCount * 0.045) * 0.04
        target_head_rot = breathe * 0.4
        target_right_arm_rot = -1.1 + breathe
        target_left_arm_rot = -0.9 - breathe

    if is_visible:
        # Fast/snappy blend while actively swinging, gentler blend otherwise -
        # this is what makes the attack recovery (and any cancel into it) feel smooth.
        smooth_amt = 0.55 if slash_active else 0.25
        cur_head_rot = lerpAngle(cur_head_rot, target_head_rot, smooth_amt)
        cur_left_arm_rot = lerpAngle(cur_left_arm_rot, target_left_arm_rot, smooth_amt)
        cur_right_arm_rot = lerpAngle(cur_right_arm_rot, target_right_arm_rot, smooth_amt)
        cur_left_leg_rot = lerpAngle(cur_left_leg_rot, target_left_leg_rot, smooth_amt)
        cur_right_leg_rot = lerpAngle(cur_right_leg_rot, target_right_leg_rot, smooth_amt)
        cur_sword_yaw = lerpAngle(cur_sword_yaw, target_sword_yaw, smooth_amt)

        drawMasterYi(cur_head_rot, cur_left_arm_rot, cur_right_arm_rot, cur_left_leg_rot, cur_right_leg_rot, cur_sword_yaw, left_arm_z, right_arm_z)
...
def updateMovementAndSkills():
    ...
    global player_speed, base_speed, slash_active, trail_history, double_strike_active
    ...
    if dx != 0.0 or dz != 0.0:
        # Moving cancels meditation AND cancels an in-progress basic attack
        if meditate_active:
            meditate_active = False
            current_anim = "idle"
        if slash_active:
...
def drawSkillEffects():
    ...
    # Passive - Double Strike embellishment: extra golden sparks on the 4th hit
    if double_strike_active:
        strokeWeight(2.5)
        for i in range(4):
            pushMatrix()
            translate(px + random(-35, 35), -40 + random(-25, 25), pz + random(-35, 35))
            rotateX(random(TWO_PI))
            rotateY(random(TWO_PI))
            stroke(255, 240, 150, 220)
            line(-22, 0, 0, 22, 0, 0)
            popMatrix()
...
def drawHUD():
    ...
    text("[A / Space / Click] Attack  |  [Arrows] Move  |  [Drag] Rotate Camera", 30, 110)
    text("Practice dummy is out at X=" + str(DUMMY_X) + ", Z=" + str(DUMMY_Z), 30, 128)

    # Passive - Double Strike combo counter
    rem = attack_count % 4
    hits_until = 4 - rem if rem != 0 else 4
    if double_strike_active:
        fill(255, 215, 0)
        text("PASSIVE — Double Strike!", 30, 146)
    else:
        fill(148, 163, 184)
        text("Passive: Double Strike in " + str(hits_until) + " hit(s)", 30, 146)
...
def keyPressed():
    ...
    global q_cooldown_timer, w_cooldown_timer, e_cooldown_timer, r_cooldown_timer
    global double_strike_active

    # Skill Bind Q - Alpha Strike (cancels an in-progress basic attack)
    if key == 'q' or key == 'Q':
        if not alpha_active and q_cooldown_timer <= 0:
            if slash_active:
                cancelSlash()
            alpha_active = True
            alpha_timer = 0
    ...
    # Skill Bind W - Meditate (cancels an in-progress basic attack)
    elif key == 'w' or key == 'W':
        if not alpha_active:
    ...
    # Skill Bind E - Wuju Style (cancels an in-progress basic attack)
    elif key == 'e' or key == 'E':
    ...
    # Skill Bind R - Highlander (cancels an in-progress basic attack)
    elif key == 'r' or key == 'R':
    ...
    # Basic attack binds: A key or Space (mouse click handled in mousePressed)
    elif key == 'a' or key == 'A' or key == ' ':
        if not slash_active and not alpha_active:
            triggerSlash()
