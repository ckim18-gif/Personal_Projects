...
class AudioSys{
 constructor(){this.b={};this.l={};this.t={};this.ready=false}
 ...
 playTracked(k){this.stopTracked(k);this.unlock();let a=this.b[k];if(!a)return;let c=a.cloneNode();c.volume=a.volume;this.t[k]=c;c.onended=()=>{if(this.t[k]===c)delete this.t[k]};c.play().catch(()=>{})}
 stopTracked(k){let a=this.t[k];if(a){a.pause();a.currentTime=0;delete this.t[k]}}
...
class Input{
 constructor(audio){this.d=new Set;this.p=new Set;this.vx=0;this.vy=0;
  addEventListener("keydown",e=>{audio.unlock();let k=e.key.toLowerCase();if(!this.d.has(k))this.p.add(k);this.d.add(k);if(["arrowup","arrowdown","arrowleft","arrowright","a","q","w","e","r","f","i","d","1","2","3","4","5"].includes(k))e.preventDefault()});
...
class Anim{constructor(img,fw,fh,n,f,loop=true,row=0){Object.assign(this,{img,fw,fh,n,f,loop,row});this.reset()}reset(){this.fr=0;this.t=0;this.done=false}u(dt,sp=1){if(this.done)return;this.t+=dt*sp;while(this.t>=1/this.f){this.t-=1/this.f;if(++this.fr>=this.n){if(this.loop)this.fr=0;else{this.fr=this.n-1;this.done=true}}}}d(c,x,y,sc=1,flip=false,a=1){c.save();c.globalAlpha=a;c.translate(Math.round(x),Math.round(y));if(flip)c.scale(-1,1);let w=this.fw*sc,h=this.fh*sc;c.drawImage(this.img,this.fr*this.fw,this.row*this.fh,this.fw,this.fh,-w/2,-h*.78,w,h);c.restore()}}
class FX{
...
class KageDashFX{
 constructor(img,x0,y0,x1,y1,dir){Object.assign(this,{img,x0,y0,x1,y1,dir});this.life=.58;this.d=.58}
 u(dt){this.life-=dt}
 d0(c,cam){let t=Math.max(0,Math.min(1,1-this.life/this.d)),ease=1-Math.pow(1-t,3),x=this.x0+(this.x1-this.x0)*ease,y=this.y0+(this.y1-this.y0)*ease,p=cam.s(x,y),fr=Math.min(6,Math.floor(t*7));c.save();c.globalAlpha=Math.min(1,t/.09)*Math.min(1,this.life/.1);c.translate(p.x,p.y);if(this.dir<0)c.scale(-1,1);c.drawImage(this.img,fr*128,5*64,128,64,-218,-112,436,218);c.restore()}
}
...
const ITEM_DATA={
 changongi:{name:"Changongi",price:800,desc:"적중 시 최대 12스택. 스택당 공속 +5%. 풀스택 방어력 +30%."},
 cheep:{name:"Cheepandsimilar",price:800,desc:"체력 20% 소모. 최대 5초간 자신을 제외한 시간 8배 감속. 재사용으로 해제."},
 ice:{name:"Ice Age",price:800,desc:"전방 직선 40 피해와 3초 빙결. 빙결 중 피격 시 조건부 처형 또는 2배 피해."},
 exesword:{name:"처형인의 검",price:800,desc:"1회용. 사용 후 다음 근접 평타 대상 1명을 즉시 처형. 처형 면역 대상은 체력 1."}
};
...
class Player{
 constructor(i,a){this.i=i;this.audio=a;this.x=V.gateX;this.y=V.y+V.h-120;this.hp=100;this.maxHp=100;this.energy=100;this.maxEnergy=100;this.gold=30;this.xp=0;this.need=35;this.level=1;this.atk=10;this.left=false;this.state="idle";this.inv=0;this.hit=false;this.med=false;this.medT=0;this.stun=0;
  ...
  this.inventory=[];this.items=[null,null,null];this.itemCd={cheep:0,ice:0};this.purchasedOnce=false;this.purchaseCount=0;this.testMode=false;this.warning="";this.warningT=0;this.slow=0;this.parryFlash=0;this.cheepOn=false;this.cheepT=0;this.cheepAfter=0;this.chStacks=0;this.chHitTimer=0;this.chDecay=0;this.afterimages=[];this.energyBuff=0;this.flameAura=0;this.onFire=0;this.onFireTick=0;this.kpvsUnlocked=false;this.doubleStrike=false;this.kpvsCd=0;this.kpvsEmpower=0;this.kpvsInv=0;this.kpvsTransform=0;this.attackCount=0;this.executeReady=false;
 ...
 moveSpeed(){return 138*(this.testMode?5:1)*(this.r.on?1.45:1)*(this.cheepOn?1.12:1)*(this.cheepAfter>0?.7:1)*(this.slow>0?.65:1)}
 ...
 stopMed(){this.med=false;this.medT=0;this.audio.stopTracked("meditate");this.state="idle"}
 itemPrice(){return this.purchaseCount===0?100:800}
 buy(id){let price=this.itemPrice();if(this.gold<price||this.inventory.length>=8){this.warning=this.inventory.length>=8?"인벤토리가 가득 찼습니다":"골드가 부족합니다";this.warningT=2;return false}this.gold-=price;this.inventory.push(id);this.purchasedOnce=true;this.purchaseCount++;for(let n=0;n<3;n++)if(!this.items[n]){this.items[n]=id;break}return true}
 enableTestMode(){this.testMode=true;this.maxHp=1000;this.hp=1000;this.maxEnergy=1000;this.energy=1000;this.gold=100000;for(let k of "qwer")this[k].open=true;this.choices=[];this.warning="테스트 모드 활성화";this.warningT=2}
 disableTestMode(){this.testMode=false;this.maxHp=100;this.hp=Math.min(this.hp,this.maxHp);this.maxEnergy=100;this.energy=Math.min(this.energy,this.maxEnergy);this.warning="테스트 모드 해제";this.warningT=2}
 ...
 useItem(slot,es,fx){
  ...
  if(id==="ice"&&this.itemCd.ice<=0){
   ...
   for(let e of es)if(!e.dead){let forward=(e.x-this.x)*dir,lateral=Math.abs(e.y-this.y);if(forward>=20&&forward<=355&&lateral<=65){if(e.takeDamage)e.takeDamage(40,this);if(!e.dead&&e.freeze)e.freeze();this.onHit()}}
  }
  if(id==="exesword"){this.executeReady=true;let idx=this.inventory.indexOf("exesword");if(idx>=0)this.inventory.splice(idx,1);this.items[slot]=null;this.warning="처형인의 검 준비: 다음 근접 평타가 대상을 처형합니다";this.warningT=2.5}
 }
 u(dt,input,es,fx){
  this.inv=Math.max(0,this.inv-dt);this.kpvsInv=Math.max(0,this.kpvsInv-dt);this.kpvsTransform=Math.max(0,this.kpvsTransform-dt);this.kpvsCd=Math.max(0,this.kpvsCd-dt);if(this.testMode)this.hp=Math.min(this.maxHp,this.hp+120*dt);this.energy=Math.min(this.maxEnergy,this.energy+(8+(this.energyBuff>0?5:0))*dt);this.energyBuff=Math.max(0,this.energyBuff-dt);this.flameAura=Math.max(0,this.flameAura-dt);this.onFire=Math.max(0,this.onFire-dt);if(this.onFire>0){this.onFireTick+=dt;if(this.onFireTick>=.5){this.onFireTick-=.5;this.hurt(1)}}this.stun=Math.max(0,this.stun-dt);this.slow=Math.max(0,this.slow-dt);this.warningT=Math.max(0,this.warningT-dt);this.parryFlash=Math.max(0,this.parryFlash-dt);if(!this.cheepOn)this.itemCd.cheep=Math.max(0,this.itemCd.cheep-dt);this.itemCd.ice=Math.max(0,this.itemCd.ice-dt);this.cheepAfter=Math.max(0,this.cheepAfter-dt);
  ...
  if(input.tap("d")&&this.kpvsUnlocked&&this.kpvsCd<=0){let x0=this.x,y0=this.y,dir=this.left?-1:1;this.kpvsCd=2;this.kpvsInv=.2;this.kpvsTransform=.58;this.kpvsEmpower=2;this.x=Math.max(30,Math.min(C.WW-30,this.x+dir*96));fx.push(new KageDashFX(this.i.kageSword,x0,y0,this.x,this.y,dir))}
  if(input.tap("1"))this.useItem(0,es,fx);if(input.tap("2"))this.useItem(1,es,fx);if(input.tap("3"))this.useItem(2,es,fx);
  if(this.stun>0)return;
  if(input.tap("w")&&this.w.open&&this.w.cd<=0&&this.energy>=15){if(this.med)this.stopMed();else{this.energy-=15;this.med=true;this.medT=0;this.w.cd=9;this.state="meditate";this.audio.playTracked("meditate")}}
  if(input.tap("e")&&this.e.open&&this.e.cd<=0&&this.energy>=25&&!this.med){this.energy-=25;this.e.cd=13;this.e.on=true;this.e.t=5;this.audio.play("wuju");fx.push(new ArrayFX(this.i.wuju2,this.x,this.y,1.35,.4))}
  ...
  if(input.tap("q")&&!this.executeReady&&this.q.open&&this.q.cd<=0&&this.energy>=20&&!this.med){let t=es.filter(e=>!e.dead&&Math.hypot(e.x-this.x,e.y-this.y)<290);if(t.length){this.energy-=20;this.q.cd=16;this.q.on=true;this.q.t=0;this.q.count=0;this.audio.play("alpha")}}
  ...
  if(this.state==="attack"){
   this.an.attack.u(dt,this.attackSpeed());
   if(!this.hit&&this.an.attack.fr>=3){
    this.hit=true;this.audio.play("attack"+(1+Math.floor(Math.random()*3)));let range=C.AR+(this.r.on?22:0),targets=es.filter(e=>!e.dead&&Math.hypot(e.x-this.x,e.y-this.y)<=range);this.attackCount++;let doubleProc=this.doubleStrike&&this.attackCount%4===0;
    for(let e of targets){
     if(this.executeReady){if(e.executeImmune){e.hp=Math.min(e.hp,1);this.showDamage(e.x,e.y,-99999,true)}else{e.hp=0;e.dead=true;this.showDamage(e.x,e.y,-99999,true)}this.executeReady=false;this.onHit();break}
     let dmg=this.atk+(this.e.on?3:0)+(this.kpvsEmpower>0?8:0),before=e.hp;
     if(e.takeDamage(dmg,this)){this.onHit();this.q.cd=Math.max(0,this.q.cd-2);if(this.flameAura>0)e.onFire=5;if(this.kpvsEmpower>0){let dealt=Math.max(0,before-e.hp);this.hp=Math.min(this.maxHp,this.hp+dealt*.5);this.energy=Math.min(this.maxEnergy,this.energy+Math.min(20,dealt));if(e.dead){this.maxHp++;this.hp++;this.maxEnergy++;this.energy++}}}
     if(doubleProc&&!e.dead)e.takeDamage(this.atk,this);
    }
    this.kpvsEmpower=0;
   }
   if(this.an.attack.done){this.state="idle";this.an.attack.reset()}return
  }
 ...
 d(c,cam){
  if(this.q.on||this.kpvsTransform>0)return;let p=cam.s(this.x,this.y);
...
class NeutralMob{
 constructor(kind,i,x,y){
  this.kind=kind;this.type=kind;this.i=i;this.x=this.homeX=x;this.y=this.homeY=y;this.dead=false;this.rewarded=false;this.aggro=false;this.state="idle";this.attackT=0;this.skillT=2;this.left=false;this.onFire=0;this.fireTick=0;this.frozen=0;this.hitDone=false;
  let q=kind==="blue"?{hp:200,power:5,gold:12,sp:68,range:46}:{hp:100,power:15,gold:15,sp:70,range:46};if(kind==="kage")q={hp:800,power:10,gold:50,sp:102,range:62};Object.assign(this,q);this.max=this.hp;
  if(kind==="kage"){this.idle=new Anim(i.kageSword,128,64,7,7,true,0);this.walk=new Anim(i.kageSword,128,64,8,10,true,2);this.attack=new Anim(i.kageSword,128,64,14,13,false,4);this.dash=new Anim(i.kageSword,128,64,7,10,false,5);this.powerAttack=new Anim(i.kageSword,128,64,7,9,false,3);this.circle=new Anim(i.kageSword,128,64,2,5,false,6);this.copy=new Anim(i.kageSword,128,64,9,10,false,7)}
  else{this.idle=new Anim(kind==="blue"?i.blueWalk:i.orangeWalk,90,64,10,7);this.walk=this.idle;this.attack=new Anim(kind==="blue"?i.blueAttack:i.orangeAttack,90,64,11,10,false)}
 }
 freeze(){if(!this.dead)this.frozen=3}
 takeDamage(n,p){if(this.dead)return false;this.aggro=true;if(this.kind==="kage"&&Math.random()<.2)return false;if(this.kind==="kage"&&Math.random()<.02){p.hurt(n);return false}this.hp-=n;p.showDamage(this.x,this.y,Math.round(n));if(p.flameAura>0&&this.kind!=="orange")this.onFire=5;if(this.hp<=0)this.dead=true;return true}
 startAttack(name="attack"){this.state=name;this.hitDone=false;this[name].reset()}
 u(dt,p,fx){
  if(this.dead)return;if(this.frozen>0){this.frozen=Math.max(0,this.frozen-dt);return}
  if(this.onFire>0){this.onFire-=dt;this.fireTick+=dt;if(this.fireTick>=.5){this.fireTick-=.5;this.hp--;if(this.hp<=0)this.dead=true}}
  let dx=p.x-this.x,dy=p.y-this.y,d=Math.hypot(dx,dy)||1,home=Math.hypot(this.x-this.homeX,this.y-this.homeY);this.left=dx<0;
  if(this.aggro&&(d>390||home>300))this.aggro=false;
  if(!this.aggro){this.state="idle";this.idle.u(dt);if(home>5){this.state="walk";this.x+=(this.homeX-this.x)/home*this.sp*dt;this.y+=(this.homeY-this.y)/home*this.sp*dt}return}
  if(this.state!=="idle"&&this.state!=="walk"){
   let a=this[this.state];a.u(dt);if(!this.hitDone&&a.fr>=3){this.hitDone=true;if(this.state==="dash"){if(d<110){p.hurt(10);this.hp=Math.min(this.max,this.hp+this.max*.1)}}else if(this.state==="powerAttack"){if(d<95){p.hurt(25);p.stun=.5}}else if(this.state==="circle"){if(d<110)p.hurt(p.hp<=30?99999:5)}else if(this.state==="copy"){if(d<150)p.hurt(8)}else if(d<this.range+18)p.hurt(this.kind==="orange"?10+Math.random()*10:this.power)}
   if(a.done){this.state="idle";a.reset()}return
  }
  this.skillT-=dt;
  if(this.kind==="kage"&&this.skillT<=0){this.skillT=1.8+Math.random()*2.2;let r=Math.random();if(r<.27){this.startAttack("dash");this.x+=dx/d*95;this.y+=dy/d*95}else if(r<.52){fx.push(new WarningFX(this.x,this.y,68,1));this.startAttack("powerAttack")}else if(r<.72&&p.hp<=p.maxHp*.5)this.startAttack("circle");else this.startAttack("copy");return}
  if(d<=this.range){this.attackT+=dt;if(this.attackT>=(this.kind==="kage"?.48:.82)){this.attackT=0;this.startAttack("attack")}}
  else{this.state="walk";this.walk.u(dt);this.x+=dx/d*this.sp*dt;this.y+=dy/d*this.sp*dt}
 }
 d(c,cam){let p=cam.s(this.x,this.y),a=this.state==="walk"?this.walk:this.state==="idle"?this.idle:this[this.state],sc=this.kind==="kage"?3.45:.9;a.d(c,p.x,p.y,sc,this.left);let by=p.y-(this.kind==="kage"?102:50);c.fillStyle="#211";c.fillRect(p.x-45,by,90,7);c.fillStyle=this.kind==="blue"?"#4aa8ff":this.kind==="orange"?"#ff8738":"#c65cff";c.fillRect(p.x-45,by,90*Math.max(0,this.hp/this.max),7)}
}
...
class Game{
 constructor(i){
  this.i=i;this.audio=new AudioSys();for(let [k,s,v,l] of [["bgm","assets/audio/bgm.ogg",.3,true],["attack1","assets/audio/attack1.wav",.55],["attack2","assets/audio/attack2.wav",.55],["attack3","assets/audio/attack3.wav",.55],["alpha","assets/audio/alpha.wav",.65],["meditate","assets/audio/meditate.wav",.5,false],["ultimate","assets/audio/ultimate.wav",.65,false],["wuju","assets/audio/attack.wav",.5,false],["level","assets/audio/levelup.wav",.6]])this.audio.add(k,s,v,l);
  this.mouseX=0;this.mouseY=0;this.click=null;const mousePos=e=>{let r=canvas.getBoundingClientRect();return{x:(e.clientX-r.left)*C.W/r.width,y:(e.clientY-r.top)*C.H/r.height}};canvas.addEventListener("mousemove",e=>{let p=mousePos(e);this.mouseX=p.x;this.mouseY=p.y});canvas.addEventListener("pointerdown",e=>{if(e.pointerType==="mouse"&&e.button!==0)return;let p=mousePos(e);this.mouseX=p.x;this.mouseY=p.y;this.click=p});this.input=new Input(this.audio);this.cam=new Cam;this.p=new Player(i,this.audio);this.damageNumbers=[];this.p.damageNumbers=this.damageNumbers;this.es=[];this.fx=[];this.proj=[];this.spawnT=0;this.kills=0;this.last=performance.now();this.shop=false;this.inventoryOpen=false;this.invSelected=0;this.mapIndex=1;this.playTime=0;this.gameOver=false;this.neutrals=[];this.neutralRespawn={blue:0,orange:0,kage:0};this.camps={blue:{x:540,y:500,img:i.blueCamp},orange:{x:1160,y:820,img:i.orangeCamp},kage:{x:1810,y:590,img:i.kageCamp}};this.gateOpen=0;this.gateFrame=0;this.bed={x:V.x+160,y:V.y+470};this.aura={x:V.gateX,y:V.y+V.h/2,r:42};
 ...
 update(dt){this.p.currentMap=this.mapIndex;if(this.gameOver){if(this.input.tap("1"))location.reload();if(this.input.tap("2")){this.gameOver=false;this.p.hp=this.p.maxHp;this.p.energy=this.p.maxEnergy;this.mapIndex=1;this.p.x=V.gateX;this.p.y=V.y+V.h-120;this.es=[];this.setupNeutrals()}return}this.playTime+=dt;
  if(this.input.tap("i")&&!this.shop&&!this.p.choices.length)this.inventoryOpen=!this.inventoryOpen;
  if(this.inventoryOpen){for(let n=1;n<=8;n++)if(this.input.tap(String(n)))this.invSelected=n-1;for(let n=0;n<8;n++){let x=105+(n%4)*110,y=95+Math.floor(n/4)*105;if(this.hitRect(x,y,90,88))this.invSelected=n}if(this.input.tap("q")||this.hitRect(205,292,70,28))this.p.equip(this.invSelected,0);if(this.input.tap("w")||this.hitRect(285,292,70,28))this.p.equip(this.invSelected,1);if(this.input.tap("e")||this.hitRect(365,292,70,28))this.p.equip(this.invSelected,2);this.clearClick();return}
  if(this.p.choices.length){for(let j=0;j<3;j++)if(this.input.tap(String(j+1))||this.hitRect(65+j*175,115,160,115))this.p.choose(j);this.clearClick();return}
  if(this.shop){let ids=["changongi","cheep","ice","exesword"];ids.forEach((id,j)=>{if(this.input.tap(String(j+1))||this.hitRect(18+j*155,72,145,210))this.p.buy(id)});if(this.input.tap("5")||this.hitRect(230,307,180,32)){if(this.p.testMode)this.p.disableTestMode();else this.p.enableTestMode()}if(this.input.tap("f")||this.hitRect(585,20,36,28))this.shop=false;this.clearClick();return}
  if(this.input.tap("f")&&Math.hypot(this.p.x-this.bed.x,this.p.y-this.bed.y)<55){this.shop=true;return}
  let gateTarget=nearGate(this.p.x,this.p.y)?1:0;this.gateOpen+=(gateTarget-this.gateOpen)*(1-Math.exp(-8*dt));this.gateFrame=Math.max(0,Math.min(5,Math.round(this.gateOpen*5)));let wdt=dt*this.worldScale();this.spawnT+=wdt;if(this.spawnT>C.SPAWN){this.spawnT=0;this.spawn()}let combatTargets=[...this.es,...this.neutrals];this.p.u(dt,this.input,combatTargets,this.fx);for(let e of this.es)e.u(wdt,this.p,this.proj,this.i,this.es);for(let n of this.neutrals)n.u(wdt,this.p,this.fx);for(let p of this.proj)p.u(wdt,this.p);for(let f of this.fx)f.u(wdt);for(let d of this.damageNumbers)d.u(dt);for(let n of this.npcs)n.u(wdt);this.proj=this.proj.filter(p=>!p.dead);this.fx=this.fx.filter(f=>f.life>0);this.damageNumbers=this.damageNumbers.filter(d=>d.life>0);this.p.damageNumbers=this.damageNumbers;
  if(this.mapIndex===1&&Math.hypot(this.p.x-this.aura.x,this.p.y-this.aura.y)<this.aura.r)this.p.gainXP(18*dt);
  ...
  if(this.p.x<=31){this.mapIndex=this.mapIndex===1?4:this.mapIndex-1;this.p.x=C.WW-35;this.es=[];this.setupNeutrals();if(this.mapIndex===1)for(let n=0;n<10;n++)this.spawn()}else if(this.p.x>=C.WW-31){this.mapIndex=this.mapIndex===4?1:this.mapIndex+1;this.p.x=35;this.es=[];this.setupNeutrals();if(this.mapIndex===1)for(let n=0;n<10;n++)this.spawn()}this.cam.follow(this.p,dt);if(this.p.hp<=0)this.gameOver=true
 }
 ground(){
  let sx=Math.floor(this.cam.x/32)*32,sy=Math.floor(this.cam.y/32)*32;
  if(this.mapIndex!==1){
   let im=this.mapIndex===2?this.i.map2:this.mapIndex===3?this.i.map3:this.i.map4;
   let patch=96,cols=Math.max(1,Math.floor(im.width/patch)),rows=Math.max(1,Math.floor(im.height/patch));
   for(let y=Math.floor(this.cam.y/patch)*patch;y<this.cam.y+C.H+patch;y+=patch)for(let x=Math.floor(this.cam.x/patch)*patch;x<this.cam.x+C.W+patch;x+=patch){
    let p=this.cam.s(x,y),gx=Math.floor(x/patch),gy=Math.floor(y/patch);
    let sx0=((gx+gy*2)%cols)*patch,sy0=((gy+Math.floor(gx/3))%rows)*patch;
    ctx.drawImage(im,sx0,sy0,Math.min(patch,im.width-sx0),Math.min(patch,im.height-sy0),p.x,p.y,patch,patch)
   }
   ctx.save();
   if(this.mapIndex===2){ctx.fillStyle="rgba(31,79,71,.12)";ctx.fillRect(0,0,C.W,C.H)}
   if(this.mapIndex===3){ctx.fillStyle="rgba(104,70,32,.16)";ctx.fillRect(0,0,C.W,C.H)}
   if(this.mapIndex===4){ctx.fillStyle="rgba(34,29,63,.24)";ctx.fillRect(0,0,C.W,C.H)}
   ctx.restore();return}
 ...
 hud(){
  ...
  let hp=this.p.hp/this.p.maxHp,xp=this.p.xp/this.p.need,en=this.p.energy/this.p.maxEnergy;ctx.fillStyle="#261315";ctx.fillRect(90,283,170,11);ctx.fillStyle="#d94f50";ctx.fillRect(92,285,166*Math.max(0,hp),7);ctx.fillStyle="#10243c";ctx.fillRect(90,296,170,9);ctx.fillStyle="#42a8ff";ctx.fillRect(92,298,166*Math.max(0,en),5);ctx.fillStyle="#15301c";ctx.fillRect(90,307,170,8);ctx.fillStyle="#57d66b";ctx.fillRect(92,309,166*Math.max(0,xp),4);ctx.fillStyle="#fff";ctx.font="bold 8px sans-serif";ctx.fillText(`HP ${Math.ceil(this.p.hp)}/${this.p.maxHp}`,95,291);ctx.fillStyle="#d9efff";ctx.fillText(`ENERGY ${Math.floor(this.p.energy)}/${this.p.maxEnergy}`,95,303);ctx.fillStyle="#dfffe5";ctx.fillText(`LV ${this.p.level} XP ${Math.floor(this.p.xp)}/${this.p.need}  G ${this.p.gold}`,90,326);
 ...
 inventoryUI(){ctx.save();ctx.fillStyle="rgba(2,5,4,.94)";ctx.fillRect(0,0,C.W,C.H);ctx.textAlign="center";ctx.fillStyle="#fff";ctx.font="bold 20px sans-serif";ctx.fillText("INVENTORY 8",320,48);ctx.font="10px sans-serif";ctx.fillText("숫자 또는 마우스로 아이템 선택 · Q/W/E 장착 · I 닫기",320,68);for(let n=0;n<8;n++){let x=105+(n%4)*110,y=95+Math.floor(n/4)*105,id=this.p.inventory[n];ctx.fillStyle=n===this.invSelected?"#31472e":"#111b14";ctx.fillRect(x,y,90,88);ctx.strokeStyle="#a8c67e";ctx.strokeRect(x+.5,y+.5,89,87);if(id){ctx.drawImage(this.i["item_"+id],x+25,y+8,40,40);ctx.fillStyle="#fff";ctx.font="9px sans-serif";ctx.fillText(ITEM_DATA[id].name,x+45,y+64)}ctx.fillStyle="#d6e5c2";ctx.fillText(String(n+1),x+10,y+14)}ctx.fillStyle="#ffe28a";ctx.fillText(`장착: ${this.p.items.map(x=>x?ITEM_DATA[x].name:"빈칸").join(" / ")}`,320,324);ctx.restore()}
 shopUI(){
  ctx.save();ctx.fillStyle="rgba(2,5,4,.94)";ctx.fillRect(0,0,C.W,C.H);ctx.fillStyle="#f3f0da";ctx.textAlign="center";ctx.font="bold 20px sans-serif";ctx.fillText("침대 상점",320,38);ctx.font="10px sans-serif";ctx.fillText(`골드 ${this.p.gold} · 첫 구매 100G / 이후 800G · F 닫기`,320,57);
  let ids=["changongi","cheep","ice","exesword"];ids.forEach((id,j)=>{let x=18+j*155,y=72,w=145,h=210,d=ITEM_DATA[id];ctx.fillStyle="#111b14";ctx.fillRect(x,y,w,h);ctx.strokeStyle="#a8c67e";ctx.strokeRect(x+.5,y+.5,w-1,h-1);ctx.drawImage(this.i["item_"+id],x+43,y+9,60,60);ctx.fillStyle="#fff";ctx.font="bold 11px sans-serif";ctx.fillText(`${j+1}. ${d.name}`,x+w/2,y+86);ctx.fillStyle="#dfc55f";ctx.fillText(`${this.p.itemPrice()} GOLD`,x+w/2,y+105);ctx.fillStyle="#b9caa9";ctx.font="8px sans-serif";let words=d.desc.split(" "),line="",yy=y+124;for(let word of words){let test=line+word+" ";if(ctx.measureText(test).width>w-12){ctx.fillText(line,x+w/2,yy);line=word+" ";yy+=11}else line=test}ctx.fillText(line,x+w/2,yy)});ctx.fillStyle="#233d59";ctx.fillRect(230,307,180,32);ctx.strokeStyle="#71b8ee";ctx.strokeRect(230.5,307.5,179,31);ctx.fillStyle="#fff";ctx.font="bold 11px sans-serif";ctx.fillText(this.p.testMode?"5. TEST MODE OFF":"5. TEST MODE ON",320,327);ctx.restore()
 }
 levelUI(){
  ctx.save();ctx.globalAlpha=1;ctx.globalCompositeOperation="source-over";ctx.fillStyle="rgba(2,5,4,.94)";ctx.fillRect(0,0,C.W,C.H);ctx.fillStyle="#fff";ctx.textAlign="center";ctx.font="bold 20px sans-serif";ctx.fillText(`LEVEL ${this.p.level}`,320,65);ctx.font="11px sans-serif";ctx.fillText("숫자키 또는 원하는 스킬 카드를 클릭",320,88);this.p.choices.forEach((o,j)=>{let x=65+j*175,y=115;ctx.fillStyle="#132017";ctx.fillRect(x,y,160,115);ctx.strokeStyle="#afca8b";ctx.strokeRect(x+.5,y+.5,159,114);ctx.fillStyle="#fff";ctx.font="bold 22px sans-serif";ctx.fillText(j+1,x+80,y+32);ctx.font="bold 13px sans-serif";ctx.fillText(o.n,x+80,y+72)});ctx.restore()
 }
