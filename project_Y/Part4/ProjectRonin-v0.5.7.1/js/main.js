...
class KageDashFX{
 constructor(img,x,y,dir){this.img=img;this.x=x;this.y=y;this.dir=dir;this.life=.32;this.d=.32}
 u(dt){this.life-=dt}
 d0(c,cam){let p=cam.s(this.x,this.y),fr=Math.min(6,Math.floor((1-this.life/this.d)*7));c.save();c.globalAlpha=Math.max(0,this.life/.08);c.translate(p.x,p.y);if(this.dir<0)c.scale(-1,1);c.drawImage(this.img,fr*256,5*64,256,64,-205,-102,410,102);c.restore()}
}
...
class Player{
 ...
 u(dt,input,es,fx){
  ...
  if(input.tap("d")&&this.kpvsUnlocked&&this.kpvsCd<=0){this.kpvsCd=2;this.kpvsInv=.2;this.kpvsEmpower=2;this.x+=this.left?-96:96;this.x=Math.max(30,Math.min(C.WW-30,this.x));fx.push(new KageDashFX(this.i.kageSword,this.x,this.y,this.left?-1:1))}
...
class NeutralMob{
 constructor(kind,i,x,y){
  ...
  if(kind==="kage"){this.idle=new Anim(i.kageSword,256,64,7,7,true,0);this.walk=new Anim(i.kageSword,256,64,7,10,true,2);this.attack=new Anim(i.kageSword,256,64,7,12,false,4);this.dash=new Anim(i.kageSword,256,64,7,14,false,5);this.powerAttack=new Anim(i.kageSword,256,64,7,10,false,4);this.circle=new Anim(i.kageSword,256,64,7,12,false,6);this.copy=new Anim(i.kageSword,256,64,7,12,false,7)}
 ...
 d(c,cam){let p=cam.s(this.x,this.y),a=this.state==="walk"?this.walk:this.state==="idle"?this.idle:this[this.state],sc=this.kind==="kage"?1.65:.9;a.d(c,p.x,p.y,sc,this.left);c.fillStyle="#211";c.fillRect(p.x-45,p.y-(this.kind==="kage"?76:50),90,7);c.fillStyle=this.kind==="blue"?"#4aa8ff":this.kind==="orange"?"#ff8738":"#c65cff";c.fillRect(p.x-45,p.y-(this.kind==="kage"?76:50),90*Math.max(0,this.hp/this.max),7)}
}
...
class Game{
 ...
 update(dt){this.p.currentMap=this.mapIndex;if(this.gameOver){if(this.input.tap("1"))location.reload();if(this.input.tap("2")){this.gameOver=false;this.p.hp=this.p.maxHp;this.p.energy=this.p.maxEnergy;this.mapIndex=1;this.p.x=V.gateX;this.p.y=V.y+V.h-120;this.es=[];this.setupNeutrals()}return}this.playTime+=dt;
  ...
  if(this.shop){if(this.input.tap("f"))this.shop=false;if(this.input.tap("1"))this.p.buy("changongi");if(this.input.tap("2"))this.p.buy("cheep");if(this.input.tap("3"))this.p.buy("ice");if(this.input.tap("4"))this.p.buy("exesword");if(this.input.tap("5")){this.p.enableTestMode();this.shop=false}return}
