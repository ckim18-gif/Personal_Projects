 ...
 useItem(slot,es,fx){
  ...
  if(id==="ice"&&this.itemCd.ice<=0){
   ...
   for(let e of es)if(!e.dead){
    ...
    if(forward>=20&&forward<=355&&lateral<=65){if(e.takeDamage)e.takeDamage(40,this);if(!e.dead&&e.freeze)e.freeze();this.onHit()}
   }
  }
 }
 u(dt,input,es,fx){
...
class NeutralMob{
 constructor(kind,i,x,y){this.kind=kind;this.type=kind;this.i=i;this.x=this.homeX=x;this.y=this.homeY=y;this.dead=false;this.rewarded=false;this.respawn=0;this.state="idle";this.attackT=0;this.skillT=2;this.left=false;this.onFire=0;this.fireTick=0;this.frozen=0;let q=kind==="blue"?{hp:200,power:5,gold:12,sp:72,range:42}:{hp:100,power:15,gold:15,sp:74,range:42};if(kind==="kage")q={hp:800,power:10,gold:50,sp:112,range:48};Object.assign(this,q);this.max=this.hp;this.img=kind==="blue"?i.blueWalk:kind==="orange"?i.orangeWalk:i.kageSword;this.anim=kind==="kage"?new Anim(this.img,256,64,7,10):new Anim(this.img,90,64,10,8);this.attack=kind==="blue"?new Anim(i.blueAttack,90,64,11,9,false):kind==="orange"?new Anim(i.orangeAttack,90,64,11,9,false):new Anim(this.img,256,64,7,14,false)}
...
class Game{
 ...
 update(dt){this.p.currentMap=this.mapIndex;if(this.gameOver){if(this.input.tap("1"))location.reload();if(this.input.tap("2")){this.gameOver=false;this.p.hp=this.p.maxHp;this.p.energy=this.p.maxEnergy;this.mapIndex=1;this.p.x=V.gateX;this.p.y=V.y+V.h-120;this.es=[];this.setupNeutrals()}return}this.playTime+=dt;
  ...
  if(this.shop){if(this.input.tap("f"))this.shop=false;if(this.input.tap("1"))this.p.buy("changongi");if(this.input.tap("2"))this.p.buy("cheep");if(this.input.tap("3"))this.p.buy("ice");if(this.input.tap("4")){this.p.enableTestMode();this.shop=false}return}
 ...
 shopUI(){
  ctx.save();ctx.globalAlpha=1;ctx.globalCompositeOperation="source-over";ctx.fillStyle="rgba(2,5,4,.94)";ctx.fillRect(0,0,C.W,C.H);ctx.fillStyle="#f3f0da";ctx.textAlign="center";ctx.font="bold 20px sans-serif";ctx.fillText("침대 상점",320,55);ctx.font="10px sans-serif";ctx.fillText(`골드 ${this.p.gold} · 첫 구매 100G / 이후 800G · F 닫기`,320,76);
  let ids=["changongi","cheep","ice"];ids.forEach((id,j)=>{let x=55+j*190,y=98,w=170,h=195,d=ITEM_DATA[id];ctx.fillStyle="#111b14";ctx.fillRect(x,y,w,h);ctx.strokeStyle="#a8c67e";ctx.strokeRect(x+.5,y+.5,w-1,h-1);ctx.drawImage(this.i["item_"+id],x+50,y+12,70,70);ctx.fillStyle="#fff";ctx.font="bold 13px sans-serif";ctx.fillText(`${j+1}. ${d.name}`,x+w/2,y+100);ctx.fillStyle="#dfc55f";ctx.fillText(`${this.p.itemPrice()} GOLD`,x+w/2,y+120);ctx.fillStyle="#b9caa9";ctx.font="9px sans-serif";let words=d.desc.split(" "),line="",yy=y+140;for(let word of words){let test=line+word+" ";if(ctx.measureText(test).width>w-18){ctx.fillText(line,x+w/2,yy);line=word+" ";yy+=12}else line=test}ctx.fillText(line,x+w/2,yy);if(this.p.has(id)){ctx.fillStyle="#77e68a";ctx.fillText("보유 중",x+w/2,y+185)}});ctx.fillStyle="#233d59";ctx.fillRect(230,310,180,32);ctx.strokeStyle="#71b8ee";ctx.strokeRect(230.5,310.5,179,31);ctx.fillStyle="#fff";ctx.font="bold 11px sans-serif";ctx.fillText("4. TEST MODE",320,330);ctx.restore()
 }
 levelUI(){
