...
class Input{
 constructor(audio){this.d=new Set;this.p=new Set;this.vx=0;this.vy=0;
  addEventListener("keydown",e=>{audio.unlock();let k=e.key.toLowerCase();if(!this.d.has(k))this.p.add(k);this.d.add(k);if(["arrowup","arrowdown","arrowleft","arrowright","a","q","w","e","r","f","i","1","2","3"].includes(k))e.preventDefault()});
...
class Enemy{
 constructor(type,i,x,y){this.type=type;this.i=i;this.x=x;this.y=y;this.dead=false;this.rewarded=false;this.state="move";this.attackTime=0;this.hitDone=false;this.left=false;this.frozen=0;this.hits=0;this.slowCd=0;this.escape=false;this.shields=type==="boss"?2:0;this.shieldT=0;
  let q={slime:{hp:30,sp:135,r:22,wind:.38,rec:.72,power:2,gold:4,sc:1.1},lancer:{hp:60,sp:140,r:38,wind:.48,rec:.82,power:1,gold:4,sc:.22},archer:{hp:1,sp:138,r:155,wind:.62,rec:.95,power:3,gold:4,sc:.25},monk:{hp:20,sp:132,r:145,wind:.7,rec:1.15,power:0,gold:4,sc:.25},pawn:{hp:1,sp:148,r:30,wind:1.0,rec:.3,power:0,gold:4,sc:.25},boss:{hp:50,sp:132,r:44,wind:.55,rec:.75,power:5,gold:12,sc:.28}}[type];Object.assign(this,q);this.max=this.hp;
  const A=(im,fw,n,f,loop=true)=>new Anim(im,fw,fw,n,f,loop);
  if(type==="slime"){this.move=new Anim(i.slimeNew,32,32,7,8);this.attack=new Anim(i.slimeNew,32,32,7,9,false)}
...
class Player{
 constructor(i,a){this.i=i;this.audio=a;this.x=V.gateX;this.y=V.y+V.h-120;this.hp=100;this.maxHp=100;this.gold=30;this.xp=0;this.need=35;this.level=1;this.atk=10;this.left=false;this.state="idle";this.inv=0;this.hit=false;this.med=false;this.medT=0;this.stun=0;
  ...
  this.inventory=[];this.items=[null,null,null];this.itemCd={cheep:0,ice:0};this.purchasedOnce=false;this.warning="";this.warningT=0;this.slow=0;this.parryFlash=0;this.cheepOn=false;this.cheepT=0;this.cheepAfter=0;this.chStacks=0;this.chHitTimer=0;this.chDecay=0;this.afterimages=[];
 ...
 hurt(n){if(this.q.on)return;let mult=1;if(this.med)mult*=.2;if(this.has("changongi")&&this.chStacks>=12)mult*=.7;if(this.cheepAfter>0)mult*=2;this.hp=Math.max(0,this.hp-n*mult);this.inv=.08}
 onHit(){
 ...
 moveSpeed(){return 155*(this.r.on?1.45:1)*(this.cheepOn?1.12:1)*(this.cheepAfter>0?.7:1)*(this.slow>0?.65:1)}
 ...
 u(dt,input,es,fx){
  this.inv=Math.max(0,this.inv-dt);this.stun=Math.max(0,this.stun-dt);this.slow=Math.max(0,this.slow-dt);this.warningT=Math.max(0,this.warningT-dt);this.parryFlash=Math.max(0,this.parryFlash-dt);if(!this.cheepOn)this.itemCd.cheep=Math.max(0,this.itemCd.cheep-dt);this.itemCd.ice=Math.max(0,this.itemCd.ice-dt);this.cheepAfter=Math.max(0,this.cheepAfter-dt);
  ...
  if(input.tap("w")&&this.w.open&&this.w.cd<=0){if(this.med)this.stopMed();else{this.med=true;this.medT=0;this.w.cd=9;this.state="meditate";this.audio.play("meditate")}}
  if(input.tap("e")&&this.e.open&&this.e.cd<=0&&!this.med){this.e.cd=13;this.e.on=true;this.e.t=5;fx.push(new ArrayFX(this.i.wuju2,this.x,this.y,1.35,.4))}
  if(input.tap("r")&&this.r.open&&this.r.cd<=0&&!this.med){this.r.cd=35;this.r.on=true;this.r.t=9;this.audio.play("ultimate");fx.push(new FX(this.i.high2,53,35,8,this.x,this.y,2,.45))}
...
class Game{
 constructor(i){
  ...
  this.input=new Input(this.audio);this.cam=new Cam;this.p=new Player(i,this.audio);this.damageNumbers=[];this.p.damageNumbers=this.damageNumbers;this.es=[];this.fx=[];this.proj=[];this.spawnT=0;this.kills=0;this.last=performance.now();this.shop=false;this.inventoryOpen=false;this.invSelected=0;this.mapIndex=1;this.gateOpen=0;this.gateFrame=0;this.bed={x:V.x+160,y:V.y+470};this.aura={x:V.gateX,y:V.y+V.h/2,r:42};
 ...
 update(dt){
  ...
  let gateTarget=nearGate(this.p.x,this.p.y)?1:0;this.gateOpen+=(gateTarget-this.gateOpen)*(1-Math.exp(-8*dt));this.gateFrame=Math.max(0,Math.min(5,Math.round(this.gateOpen*5)));let wdt=dt*this.worldScale();this.spawnT+=wdt;if(this.spawnT>C.SPAWN){this.spawnT=0;this.spawn()}this.p.u(dt,this.input,this.es,this.fx);for(let e of this.es)e.u(wdt,this.p,this.proj,this.i,this.es);for(let p of this.proj)p.u(wdt,this.p);for(let f of this.fx)f.u(wdt);for(let d of this.damageNumbers)d.u(dt);for(let n of this.npcs)n.u(wdt);this.proj=this.proj.filter(p=>!p.dead);this.fx=this.fx.filter(f=>f.life>0);this.damageNumbers=this.damageNumbers.filter(d=>d.life>0);this.p.damageNumbers=this.damageNumbers;
  if(Math.hypot(this.p.x-this.aura.x,this.p.y-this.aura.y)<this.aura.r)this.p.gainXP(18*dt);
  for(let e of this.es)if(e.dead&&!e.rewarded){e.rewarded=true;this.kills++;this.p.reward(e.gold||4,e.type==="boss"?35:10)}
  if(this.p.x<=31){this.mapIndex=this.mapIndex===1?4:this.mapIndex-1;this.p.x=C.WW-35;this.es=[]}else if(this.p.x>=C.WW-31){this.mapIndex=this.mapIndex===4?1:this.mapIndex+1;this.p.x=35;this.es=[]}this.cam.follow(this.p,dt);if(this.p.hp<=0)location.reload()
 }
 ground(){
  let sx=Math.floor(this.cam.x/32)*32,sy=Math.floor(this.cam.y/32)*32;
  if(this.mapIndex!==1){let im=this.mapIndex===2?this.i.map2:this.mapIndex===3?this.i.map3:this.i.map4,tw=32;for(let y=sy;y<this.cam.y+C.H+tw;y+=tw)for(let x=sx;x<this.cam.x+C.W+tw;x+=tw){let p=this.cam.s(x,y),cols=Math.max(1,Math.floor(im.width/tw)),rows=Math.max(1,Math.floor(im.height/tw)),h=Math.abs((Math.floor(x/96)*31)^(Math.floor(y/96)*17)),sx0=(h%cols)*tw,sy0=((Math.floor(h/cols))%rows)*tw;ctx.drawImage(im,sx0,sy0,tw,tw,p.x,p.y,tw,tw)}return}
 ...
 draw(){
  ...
  let list=[];for(let b of this.buildings)list.push({y:b.y,d:()=>{let p=this.cam.s(b.x,b.y);ctx.drawImage(b.img,p.x-b.w/2,p.y-b.h,b.w,b.h)}});let bedp=this.cam.s(this.bed.x,this.bed.y);list.push({y:this.bed.y,d:()=>ctx.drawImage(this.i.bedNew,32,48,64,48,bedp.x-40,bedp.y-44,80,60)});for(let n of this.npcs)list.push({y:n.y,d:()=>n.d(ctx,this.cam)});for(let e of this.es)if(!e.dead)list.push({y:e.y,d:()=>e.d(ctx,this.cam)});list.push({y:this.p.y,d:()=>this.p.d(ctx,this.cam)});list.sort((a,b)=>a.y-b.y);for(let o of list)o.d();for(let p of this.proj)p.d(ctx,this.cam);for(let f of this.fx)f.d0(ctx,this.cam);for(let d of this.damageNumbers)d.d(ctx,this.cam);
  let sd=Math.hypot(this.p.x-this.bed.x,this.p.y-this.bed.y);ctx.save();ctx.textAlign="center";ctx.fillStyle="rgba(255,221,91,.22)";ctx.beginPath();ctx.arc(bedp.x,bedp.y-22,24+Math.sin(performance.now()/170)*3,0,Math.PI*2);ctx.fill();ctx.fillStyle="#ffe587";ctx.font="bold 10px sans-serif";ctx.fillText(sd<55?"F 상점 열기":"상점",bedp.x,bedp.y-54);if(sd>=55){let a=Math.atan2(this.bed.y-this.p.y,this.bed.x-this.p.x),px=320+Math.cos(a)*125,py=170+Math.sin(a)*92;ctx.translate(px,py);ctx.rotate(a);ctx.beginPath();ctx.moveTo(11,0);ctx.lineTo(-7,-7);ctx.lineTo(-7,7);ctx.closePath();ctx.fill()}ctx.restore()
  this.hud();if(this.shop)this.shopUI();if(this.inventoryOpen)this.inventoryUI();if(this.p.choices.length)this.levelUI()
 }
 hud(){
  ...
  if(this.p.warningT>0){ctx.textAlign="center";ctx.fillStyle="#ff7676";ctx.font="bold 12px sans-serif";ctx.fillText(this.p.warning,320,260)}ctx.textAlign="right";ctx.fillStyle="#fff";ctx.fillText(`MAP ${this.mapIndex}`,625,20);ctx.restore()
 }
