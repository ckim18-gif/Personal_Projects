...
class Game{
 constructor(i){
  ...
  this.input=new Input(this.audio);this.cam=new Cam;this.p=new Player(i,this.audio);this.es=[];this.fx=[];this.proj=[];this.spawnT=0;this.kills=0;this.last=performance.now();this.shop=false;this.gateOpen=0;this.gateFrame=0;this.bed={x:V.x+160,y:V.y+470};this.aura={x:V.gateX,y:V.y+V.h/2,r:42};
 ...
 ground(){
  ...
  const tile=(n)=>tiles[n%tiles.length]||this.i.grass;
  for(let y=sy;y<this.cam.y+C.H+16;y+=16)for(let x=sx;x<this.cam.x+C.W+16;x+=16){
    let p=this.cam.s(x,y),cx=Math.floor(x/64),cy=Math.floor(y/64),hash=Math.abs((cx*73856093)^(cy*19349663));
    ctx.drawImage(this.i.grass,p.x,p.y,16,16);
    if(!inVillage(x,y)){if(hash%6===0)ctx.drawImage(tile(hash),p.x,p.y,16,16)}
    else{
      let mainRoad=Math.abs(x-V.gateX)<48, plaza=y>V.y+145&&y<V.y+215;
      if(mainRoad||plaza)ctx.drawImage(tile(10),p.x,p.y,16,16);
      else if((Math.floor(x/48)+Math.floor(y/48))%7===0)ctx.drawImage(tile(3),p.x,p.y,16,16);
      else{ctx.fillStyle="rgba(190,157,96,.08)";ctx.fillRect(p.x,p.y,16,16)}
    }
  }
  // wooden fence perimeter with gate gap
  let tl=this.cam.s(V.x,V.y),tr=this.cam.s(V.x+V.w,V.y),bl=this.cam.s(V.x,V.y+V.h),br=this.cam.s(V.x+V.w,V.y+V.h),lg=this.cam.s(V.gateX-V.gateW/2,V.y+V.h),rg=this.cam.s(V.gateX+V.gateW/2,V.y+V.h);
  ctx.save();ctx.strokeStyle="#6a4825";ctx.lineWidth=8;ctx.lineCap="square";ctx.beginPath();ctx.moveTo(tl.x,tl.y);ctx.lineTo(tr.x,tr.y);ctx.lineTo(br.x,br.y);ctx.lineTo(rg.x,rg.y);ctx.moveTo(lg.x,lg.y);ctx.lineTo(bl.x,bl.y);ctx.lineTo(tl.x,tl.y);ctx.stroke();ctx.strokeStyle="#a4743b";ctx.lineWidth=2;
  for(let x=V.x;x<=V.x+V.w;x+=28){for(let y of [V.y,V.y+V.h]){if(y===V.y+V.h&&Math.abs(x-V.gateX)<V.gateW/2)continue;let a=this.cam.s(x,y-7),b=this.cam.s(x,y+7);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke()}}
  for(let y=V.y;y<=V.y+V.h;y+=28){for(let x of [V.x,V.x+V.w]){let a=this.cam.s(x-7,y),b=this.cam.s(x+7,y);ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke()}}ctx.restore();
  // 6 x 64px frame gate strip
  let gp=this.cam.s(V.gateX,V.y+V.h+5);ctx.drawImage(this.i.gate,this.gateFrame*64,0,64,64,gp.x-48,gp.y-58,96,64)
 }
 draw(){
