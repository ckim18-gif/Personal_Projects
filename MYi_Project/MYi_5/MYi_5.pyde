...
def drawHUD():
    ...
    elif e_cooldown_timer > 0:
        fill(148, 163, 184)
        text("[E] Wuju Style - Cooldown (" + str(round(e_cooldown_timer / 60.0, 1)) + "s)", 30, 160)
    else:
        fill(226, 232, 240)
    ...
    elif r_cooldown_timer > 0:
        fill(148, 163, 184)
        text("[R] Highlander - Cooldown (" + str(round(r_cooldown_timer / 60.0, 1)) + "s)", 30, 175)
    else:
        fill(226, 232, 240)
        text("[R] Highlander (Activate Speed Ghost Trails)", 30, 175)

    fill(148, 163, 184)
    q_status = "READY" if q_cooldown_timer <= 0 else str(round(q_cooldown_timer / 60.0, 1)) + "s"
    w_status = "READY" if w_cooldown_timer <= 0 else str(round(w_cooldown_timer / 60.0, 1)) + "s"
    text("[Q] Alpha Strike: " + q_status + "   [W] Meditate: " + w_status, 30, 190)
...
def keyPressed():
    ...
    global q_cooldown_timer, w_cooldown_timer, e_cooldown_timer, r_cooldown_timer   # 추가
    ...
    # Skill Bind W - Meditate
        ...
        if not alpha_active and not slash_active:
            if not meditate_active and w_cooldown_timer <= 0:
                ...
                meditate_active = False
                current_anim = "idle"
                w_cooldown_timer = w_cooldown_max   # 명상 끝낼 때 쿨타임 시작
