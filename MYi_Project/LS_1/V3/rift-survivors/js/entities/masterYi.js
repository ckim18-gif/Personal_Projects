...
class MasterYi extends Entity {
      ...
      for (const hit of this.alphaHits) {
        if (!hit.done && this.alphaTimer >= hit.time) {
          ...
          if (hit.target && !hit.target.dead) {
            ...
            const slashAngle = Utils.randRange(-0.3, 0.3) - Math.PI / 2; // 대략 위→아래 광검 느낌
            game.slashes.push(new SlashEffect(hit.target.x, hit.target.y, Utils.randRange(0, Math.PI * 2), { color: "#eaffff", length: 42 }));
            game.imageSlashes.push(new ImageSlashEffect(hit.target.x, hit.target.y, randomSlashImage(), { angle: slashAngle, height: 130 }));
  ...
  castQ(game) {
    ...
    game.imageSlashes.push(new ImageSlashEffect(this.x, this.y, randomSlashImage(), { angle: -Math.PI / 2, height: 170, life: 0.5 }));
