...
class Game {
  constructor() {
    ...
    // 진짜 도트게임처럼 보이게 하기 위한 저해상도 렌더 버퍼.
    // 모든 게임 화면을 작은 내부 해상도에 그린 뒤, 정수배로 확대(스무딩 없이)해서
    // 화면 전체가 하나의 균일한 픽셀 격자 위에서 그려지도록 만든다.
    // (원형/그라디언트 등도 이 버퍼를 거치면서 계단현상이 생겨 '오려붙인 이미지' 느낌이 사라짐)
    this.RENDER_SCALE = 3;
  ...
  resize() {
    ...
    this.buffer.width = Math.max(160, Math.round(window.innerWidth / this.RENDER_SCALE));
    this.buffer.height = Math.max(90, Math.round(window.innerHeight / this.RENDER_SCALE));
    this.bctx.imageSmoothingEnabled = false;
  }
  ...
  bindInput() {
    ...
    this.canvas.addEventListener("mousemove", (e) => {
      ...
      this.mouse.wx = e.clientX / this.RENDER_SCALE + this.cam.x;
      this.mouse.wy = e.clientY / this.RENDER_SCALE + this.cam.y;
    });
  }
  ...
  update(dt) {
    ...
    // 카메라: 플레이어 중심. 맵이 버퍼(내부 해상도)보다 좁으면 클램프하지 않고 항상 플레이어를 중앙에 둔다
    this.cam.x = this.player.x - this.buffer.width / 2;
    this.cam.y = this.player.y - this.buffer.height / 2;
    if (this.currentMode.bounds) {
      const b = this.currentMode.bounds;
      const boundW = b.maxX - b.minX, boundH = b.maxY - b.minY;
      if (boundW > this.buffer.width) {
        this.cam.x = Utils.clamp(this.cam.x, b.minX - 60, b.maxX - this.buffer.width + 60);
      }
      if (boundH > this.buffer.height) {
        this.cam.y = Utils.clamp(this.cam.y, b.minY - 60, b.maxY - this.buffer.height + 60);
      }
    }
  ...
  render() {
    ...
    let drawCam = this.cam;
    if (this.shakeTime > 0) {
      drawCam = {
        x: this.cam.x + Utils.randRange(-this.shakeMag, this.shakeMag),
        y: this.cam.y + Utils.randRange(-this.shakeMag, this.shakeMag)
      };
    }
    ...
    // 저해상도 버퍼를 화면 전체 크기로 정수배 확대(스무딩 OFF) - 진짜 도트게임 느낌의 핵심
    this.ctx.imageSmoothingEnabled = false;
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    this.ctx.drawImage(buf, 0, 0, buf.width, buf.height, 0, 0, this.canvas.width, this.canvas.height);

    // 확대된 화면 위에 부드러운 비네트(분위기 보정)만 별도로 얹는다
