...
class Game {
  ...
  bindInput() {
    window.addEventListener("keydown", (e) => {
      ...
      if (k === "z") this.player.performBasicAttack(this);
    });
  ...
  render() {
    ...
    this.currentMode.drawBackground(ctx, this.cam);

    for (const s of this.structures) s.draw(ctx, this.cam);
    for (const m of this.minions) m.draw(ctx, this.cam);
    for (const jm of this.jungleMonsters) jm.draw(ctx, this.cam);
    if (this.darius && this.darius.alive) this.darius.draw(ctx, this.cam);
    if (this.player) this.player.draw(ctx, this.cam);

    for (const tg of this.telegraphs) tg.draw(ctx, this.cam);
    for (const p of this.particles) p.draw(ctx, this.cam);
    for (const s of this.slashes) s.draw(ctx, this.cam);
    for (const f of this.floatTexts) f.draw(ctx, this.cam);
  }
}
